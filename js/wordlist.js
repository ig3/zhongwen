...
const showZhuyin = localStorage.zhuyin === 'yes';
...
$(document).ready(function () {
  ...
  const wordsElement = $('#words');
  let invalidateRow;
  const table = wordsElement.DataTable({
    data: entries,
    columns: [
      { data: 'id' },
      { data: 'simplified' },
  ...
  wordsElement.find('tbody').on('click', 'tr', function (event) {
    if (!event.target._DT_CellIndex || event.target._DT_CellIndex.column === NOTES_COLUMN) {
      const index = event.currentTarget._DT_RowIndex;
  ...
  $('#saveNotes').click(() => {
    const entry = entries[$('#rowIndex').val()];
  ...
  $('#saveList').click(function () {
    const selected = table.rows('.bg-info').data();
